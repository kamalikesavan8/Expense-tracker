...
function App() {
  ...
  const handleDeleteTransaction = (id) => {
    ...
    setLastdeleted((prev)=>[...prev,deletedItem]);
...
const handleUndo = () =>{
  ...
  const lastItem = lastdeleted[lastdeleted.length-1];
    dispatch({type:"ADD",payload: lastItem,});
    setLastdeleted((prev)=> prev.slice(0,-1));
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-6">
        ...
        {filteredTransactions.map((t) => (
          <div key={t.id} className="flex justify-between bg-gray-100 p-2 rounded-lg mb-2 hover:scale-105 transition">
            ...
            <span className="text-xs font-bold text-gray-400">{t.date} • {t.time}</span>
            ...
            <div className="bg-yellow-100 p-3 mt-3 flex justify-between items-center rounded">
